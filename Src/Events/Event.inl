...
template<typename T>
impl::EventImpl<T>::EventImpl(const std::function<void(const T &)> & _function)
	: function(_function)
	, blocked(false)
	, disconnected(false)
{
...
{
    m_events.push_back(std::make_unique<impl::EventImpl<T>>(function));
...
template<typename T>
EventHolder<T>::EventHolder(impl::EventImpl<T> * e)
	: m_eventImpl(e)
{
}

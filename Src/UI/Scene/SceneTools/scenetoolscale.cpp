...
constexpr float arrowSize2 = 3;
constexpr float squareSize = 5;

SceneToolScale::SceneToolScale(SceneNode *node)
    : SceneToolBase(node)
    , m_pressed(false)
{
...
bool SceneToolScale::mouseMoveEvent(QMouseEvent * event)
{
    if(!m_pressed)
        return false;
    ...
    if(m_directionState == DirectionState::Vertical)
        offset.x = 0;
    m_node->getLocalTransform().scale = m_baseScale + offset;

    return false;
}

bool SceneToolScale::mousePressEvent(QMouseEvent * event)
{
    ...
    m_baseScale = m_node->getLocalTransform().scale;
...
bool SceneToolScale::mouseReleaseEvent(QMouseEvent * event)
{
    if(event->button() != Qt::LeftButton)
        return false;
    m_pressed = false;
...
void SceneToolScale::draw(sf::RenderTarget &target, sf::RenderStates) const
{
    if(m_node == nullptr)
        return;

    sf::VertexArray line(sf::Lines, 24);
    ...
    line[2].position = sf::Vector2f(arrowSize - 2*arrowSize2, -arrowSize2);
    line[3].position = sf::Vector2f(arrowSize - 2*arrowSize2, arrowSize2);

    line[4].position = sf::Vector2f(arrowSize, arrowSize2);
    line[5].position = sf::Vector2f(arrowSize, -arrowSize2);

    line[6].position = sf::Vector2f(arrowSize, arrowSize2);
    line[7].position = sf::Vector2f(arrowSize - 2*arrowSize2, arrowSize2);

    line[8].position = sf::Vector2f(arrowSize, -arrowSize2);
    line[9].position = sf::Vector2f(arrowSize - 2*arrowSize2, -arrowSize2);

    for(int i(0) ; i < 10 ; i++)
        line[i].color = horizontalColor;

    //vertical arrow
    line[10].position = sf::Vector2f(0, 0);
    line[11].position = sf::Vector2f(0, arrowSize);

    line[12].position = sf::Vector2f(-arrowSize2, arrowSize - 2*arrowSize2);
    line[13].position = sf::Vector2f(arrowSize2, arrowSize - 2*arrowSize2);

    line[14].position = sf::Vector2f(arrowSize2, arrowSize);
    line[15].position = sf::Vector2f(-arrowSize2, arrowSize);

    line[16].position = sf::Vector2f(arrowSize2, arrowSize);
    line[17].position = sf::Vector2f(arrowSize2, arrowSize - 2*arrowSize2);

    line[18].position = sf::Vector2f(-arrowSize2, arrowSize);
    line[19].position = sf::Vector2f(-arrowSize2, arrowSize - 2*arrowSize2);
    ...
    line[20].position = sf::Vector2f(0, squareSize);
    line[21].position = sf::Vector2f(squareSize, squareSize);

    line[22].position = sf::Vector2f(squareSize, 0);
    line[23].position = sf::Vector2f(squareSize, squareSize);

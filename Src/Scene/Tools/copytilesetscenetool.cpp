...
void CopyTilesetSceneTool::onAddTile(const sf::Vector2u & pos)
{
    if(!m_onSelection)
    {
        m_onSelection = true;
...
void CopyTilesetSceneTool::drawCursor(sf::RenderTarget &target, const sf::Vector2u & pos) const
{
...
void CopyTilesetSceneTool::beforeSelectionEnd()
{
    m_onSelection = false;
    m_haveSelected = true;
}

void CopyTilesetSceneTool::onCopy(const CopyEvent &)
{
...
void CopyTilesetSceneTool::onCut(const CutEvent &)
{
    if(m_onSelection || !m_haveSelected)
        return;
